...
-(void)addTransactionObserver{
    NSLog(@"iap----VAILDATING_RECEIPTS_URL=%@",VAILDATING_RECEIPTS_URL);
...
-(void)sendToServer:(SKPaymentTransaction *)transaction
{
     NSLog(@"sendToServer---");
	...
	NSString *recepit = [GTMBase64IOS stringByEncodingData:transaction.transactionReceipt];
...
//    NSString* msg = [writer stringWithObject:data];
    //NSLog(@"verifyReceipt：%@",msg);
	[_transactionDelegate didSendTransactionToServer:request.productIdentifier withMsg:recepit];
}
...
//	NSString * encodedString =  [recepit stringByReplacingOccurrencesOfString:@"+" withString:@"%2B"];  
	NSDictionary* data = [NSDictionary dictionaryWithObjectsAndKeys:recepit, @"receipt-data", nil];
	SBJsonWriter *writer = [SBJsonWriter new];
    NSLog(@"verifyReceipt：%@",[writer stringWithObject:data]);
