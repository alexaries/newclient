...
{
    NSLog(@"iaphander---initECPurchaseWithHandler");
    @synchronized(self)     {
        if (!DefaultHandle_) {
...
{
    NSLog(@"didReceivedProducts---%d",products.count);
...
{
     NSLog(@"didFailedTransaction---%@",proIdentifier);
    //   NSLog(@"交易取消");
...
-(void)didSendTransactionToServer:(NSString *)proIdentifier withMsg:(NSString *)msg
{
    NSLog(@"didSendTransactionToServer---%@",proIdentifier);

     NSDictionary* data = [NSDictionary dictionaryWithObjectsAndKeys:msg, @"msg", nil];
    [[NSNotificationCenter defaultCenter] postNotificationName:IAPDidSendTransactionToServer object:proIdentifier userInfo:data];
}
...
{
    //写下你的逻辑， 加金币 or 解锁 or ...
    NSLog(@"afterProductBuyed---%@",proIdentifier);
