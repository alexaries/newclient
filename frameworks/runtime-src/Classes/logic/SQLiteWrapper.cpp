...
void SQLiteWrapper::closeSqlite(){
    log("closeSqlite---");
    if(db_ != NULL){
        sqlite3_close(db_);
    }
...
std::string SQLiteWrapper::initializing(const std::string &db_file,const std::string &searchPath,const std::string &searchResolution){
    ...
    std::string path = CCFileUtils::getInstance()->getWritablePath() + db_file;
    //bool result = this->open(path.c_str());
    fullpath = path;
    
    cocos2d::log("db path=%s",fullpath.c_str());
