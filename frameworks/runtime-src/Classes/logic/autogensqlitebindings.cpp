...
bool js_autogensqlitebindings_SQLiteWrapper_closeSqlite(JSContext *cx, uint32_t argc, jsval *vp)
{
	JSObject *obj = JS_THIS_OBJECT(cx, vp);
	js_proxy_t *proxy = jsb_get_js_proxy(obj);
	SQLiteWrapper* cobj = (SQLiteWrapper *)(proxy ? proxy->ptr : NULL);
	JSB_PRECONDITION2( cobj, cx, false, "js_autogensqlitebindings_SQLiteWrapper_closeSqlite : Invalid Native Object");
	if (argc == 0) {
		cobj->closeSqlite();
	...
	JS_ReportError(cx, "js_autogensqlitebindings_SQLiteWrapper_closeSqlite : wrong number of arguments: %d, was expecting %d", argc, 0);
	return false;
}
...
void js_register_autogensqlitebindings_SQLiteWrapper(JSContext *cx, JSObject *global) {
	...
	static JSFunctionSpec funcs[] = {
		...
		JS_FN("closeSqlite", js_autogensqlitebindings_SQLiteWrapper_closeSqlite, 0, JSPROP_PERMANENT | JSPROP_ENUMERATE),
