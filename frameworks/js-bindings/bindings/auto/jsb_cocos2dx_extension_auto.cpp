...
bool js_cocos2dx_extension_ScrollView_unscheduleAll(JSContext *cx, uint32_t argc, jsval *vp)
{
	JSObject *obj = JS_THIS_OBJECT(cx, vp);
	js_proxy_t *proxy = jsb_get_js_proxy(obj);
	cocos2d::extension::ScrollView* cobj = (cocos2d::extension::ScrollView *)(proxy ? proxy->ptr : NULL);
	JSB_PRECONDITION2( cobj, cx, false, "js_cocos2dx_extension_ScrollView_unscheduleAll : Invalid Native Object");
	if (argc == 0) {
		cobj->unscheduleAll();
		JS_SET_RVAL(cx, vp, JSVAL_VOID);
		return true;
	...
	JS_ReportError(cx, "js_cocos2dx_extension_ScrollView_unscheduleAll : wrong number of arguments: %d, was expecting %d", argc, 0);
	return false;
}
...
void js_register_cocos2dx_extension_ScrollView(JSContext *cx, JSObject *global) {
	...
	static JSFunctionSpec funcs[] = {
		...
		JS_FN("unscheduleAll", js_cocos2dx_extension_ScrollView_unscheduleAll, 0, JSPROP_PERMANENT | JSPROP_ENUMERATE),
